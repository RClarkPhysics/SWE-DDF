...
class Gaus_Poly1_TDE:    
    ...
    """
    def FuncApproxF(self,Xdata,length,C,beta,R,D,copy,tau):
        ...
        XtauT = XTau.T
        Ydata = self.GenY(XTau[0:D],length,D)
        NcLength = len(C)
    ...
    def PredictIntoTheFuture(self,F,PreLength,PData):
        #PData must go from time 0 to tau*(D) and be D dimensional (D by tau*D)
